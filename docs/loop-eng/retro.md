...
- 2026-08-12 v2-full: **`elementsFromPoint` alone under-reports contrast on react-native-web.** RN-Web sets `pointer-events:none` on a disabled `Pressable`, so the element drops out of the hit-stack and text measures as a bogus 1:1 bg-on-bg — produced 3 false failures before the maker unioned the point-stack with the element's ancestor chain and ordered by DOM depth. Promoted: any contrast probe on this stack must composite the ancestor chain, not the hit-stack.
- 2026-08-12 v2-full: **a seeded-data module is a correctness surface, not fixtures.** Slice 13 hand-typed KPI deltas, heat-grid values and watch-area counts alongside a trend series, and four groups contradicted each other in one frame. Prose ("figures only from specs §9") does not catch self-inconsistency, because every number was individually plausible. Fix pattern: one base array, everything else DERIVED from it in code. Candidate census test if this recurs: assert cross-consistency of any seeded dataset a screen renders more than one view of.
